CREATE TABLE `task` (
  ...
  `task_dimension` tinyint(1) DEFAULT '0' COMMENT '任务纬度',
  `total_thread` tinyint(11) DEFAULT '0' COMMENT '总线程数',
  `retry_count` tinyint(1) DEFAULT '0' COMMENT '任务重试次数',
  `alert_url` varchar(1024) DEFAULT '' COMMENT '报警url',
...
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=utf8mb4 COMMENT='故障重试-总快照表'

CREATE TABLE `task_segement` (
  `id` int(11) unsigned NOT NULL AUTO_INCREMENT,
  `task_id` varchar(32) NOT NULL DEFAULT '' COMMENT '任务唯一id',
  `snapshot_value` varchar(1024) DEFAULT '' COMMENT '任务快照json',
  `status` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '任务状态',
  `biz_id` varchar(100) DEFAULT '' COMMENT '业务id',
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4 COMMENT='故障重试-子任务表'
